...
OBS Studio 的图形渲染线程是整个视频系统的核心引擎，负责每一帧的定时推进、场景渲染、格式转换、编码输出和显示刷新。这篇文章是我读 `obs-video.c` 和相关文件的笔记，从源码层面把图形渲染线程的工作流程完整走一遍。

本文基于 OBS Studio 主分支源码，核心文件为：
- `libobs/obs-video.c` — 图形线程主循环
- `libobs/obs-source.c` — 源的渲染逻辑
...
**canvas（obs_canvas）** —— 画布，封装了 view + mix：
```c
struct obs_canvas {
    struct obs_view view;              // 内容
    struct obs_core_video_mix *mix;    // 管线
...
```
- 一个 view 可以绑定多个 mix（同内容不同分辨率输出）
- 不同 view 配不同 mix 则是完全不同内容的输出（如虚摄像头输出独立场景）
- 不同源完全可以画在一起——这就是 view 的 channels 做的事
...
static uint64_t tick_sources(uint64_t cur_time, uint64_t last_time)
{
    ...
    seconds = (float)(delta_time / 1000000000.0);
    ...
    return cur_time;  // 存入 context->last_time，供下一帧计算 deltaTime
}
```

每个源的 tick 中会做的事情：
- **过渡源**：推进动画时间（`obs_transition_tick`）
- **异步视频源**：计算当前应显示哪个解码帧（`async_tick`）
- **可控媒体源**：处理播放/暂停/停止等媒体操作
- **延迟属性更新**：应用保存的源属性变更
- **显隐检测**：根据引用计数变化触发 show/hide 事件

`tick_sources` 只是调度器/分发器——算 deltaTime → 遍历源 → 各调各的。具体逻辑在 source 实现层。
...
#### 7.3.2 `render_output_texture()` — 缩放
...
#### 7.3.3 `render_convert_texture()` — 颜色空间转换（可选）
...
#### 7.3.4 `output_gpu_encoders()` — 喂给硬件编码器（可选）

将 `convert_textures_encode[]` 中的 GPU 纹理直接传给 NVENC/AMF，**零拷贝**，编码器直接从显存读取。

#### 7.3.5 `stage_output_texture()` — 拷贝到 staging surface（可选）
...
| 文件 | 内容 |
|---|---|
| `libobs/obs-video.c` | 图形线程主循环、render_video、tick_sources、output_frame |
| `libobs/obs-source.c` | 源的递归渲染树、渲染回调分发 |
| `libobs/obs-view.c` | view 的 channel 管理和渲染 |
...
| `libobs/obs-internal.h` | obs_core_video_mix、obs_view、obs_core_video 结构体定义 |
