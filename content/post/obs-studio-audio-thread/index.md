...
description: "从 audio_output_open 到 input_and_output，完整梳理 OBS Studio 音频子系统：核心音频线程、6 mix × 8 channel 架构、动态缓冲机制、以及多平台采集线程的数据流全景。"
date: 2026-07-16
...
前两篇分别梳理了主线程启动流程和图形渲染管线，这篇来看音频子系统。OBS 常说的"三大线程"之外，音频其实也有一套完整的线程体系——一个核心音频处理线程加上各个平台的音频采集线程。这篇文章是我在啃音频相关源码时的学习笔记，保留了"边问边学"的过程感，希望能帮到同样对 OBS 音频架构感兴趣的人。
...
读音频子系统的这几天，最大的感受是 OBS 的音频架构和视频管线在思路上高度对称——都有独立的核心时钟线程、都有按需消费的机制、都用 deque 做缓冲解耦。几个关键设计值得记住：
