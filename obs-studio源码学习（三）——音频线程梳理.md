...
> 本文是 obs-studio 源码学习的第三篇笔记，以初学者的第一视角记录了音频子系统的学习过程。全篇围绕"数据从哪来、经过谁、到哪去"这条主线，把采集线程、音频树构建、混音渲染、缓冲管理等细节逐一拆开。建议配合前两篇一起看。

---

## 零、先圈定范围：OBS 里到底有哪些线程跟音频相关

OBS 常说的"三大线程"是 main（UI）、graphics（视频渲染）、video encoder（视频编码）。音频这边有自己的线程体系：

**全局 1 个核心线程：**

| 线程名 | 文件:行号 | 职责 |
|--------|----------|------|
| `audio-io: audio thread` | `libobs/media-io/audio-io.c:376` | 音频 Tick 时钟、混音渲染、分发给编码器/监控 |

**平台采集线程（每个 source 可能有自己的一套）：**

| 线程名 | 文件:行号 | 何时创建 |
|--------|----------|---------|
| `win-wasapi: capture thread` | `plugins/win-wasapi/win-wasapi.cpp:416` | WASAPI source 构造（RTWQ 不可用时） |
| `win-wasapi: reconnect thread` | `plugins/win-wasapi/win-wasapi.cpp:594` | Source activate 时 |
| CoreAudio reconnect thread | `plugins/mac-capture/mac-audio.c:476` | 设备断开通知到达时 |
| ALSA listen 线程 | `plugins/linux-alsa/alsa-input.c:385` | Source 创建时 |
| ALSA reopen 线程 | `plugins/linux-alsa/alsa-input.c:513` | 设备打开失败时 |
| OSS reader 线程 | `plugins/oss-audio/oss-input.c:281` | Source 创建时 |
| PulseAudio mainloop（PA 内部） | `plugins/linux-pulseaudio/pulse-wrapper.c:103` | 首次 `pulse_init()` 时 |
| PulseAudio mainloop for 监控（PA 内部） | `libobs/audio-monitoring/pulse/pulseaudio-wrapper.c:173` | 首次 `pulseaudio_init()` 时 |

**不是线程但值得一提：** 音频编码器和音频监控**没有自己的线程**——它们通过回调在 audio-io 线程上同步执行。

架构上就是**多生产者-单消费者**：N 个采集线程各自往各自 source 的 deque 里 push 数据，1 个 audio-io 线程每 Tick 从所有 deque 里 pop 数据出来混音分发。
